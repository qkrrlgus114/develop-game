  ...
  return (
    <main className="page-shell page-shell--landing">
      ...
      <section className="hero-panel">
        <p className="hero-panel__eyebrow">Vintage 업무 혼돈 시뮬레이터</p>
        ...
        <div className="hero-panel__actions">
          <VintageButton>게임 시작</VintageButton>
          <VintageButton variant="secondary">플레이 방법 보기</VintageButton>
        </div>
      </section>

      <section className="three-column-grid" aria-label="핵심 규칙 요약">
        <VintagePanel title="승리 조건" eyebrow="Rule 01">
          <p>핵심 체크리스트를 끝내고 실제 방해 요소를 처리하면 클리어입니다.</p>
        </VintagePanel>
        <VintagePanel title="오판 패널티" eyebrow="Rule 02">
          <p>가짜 팝업에 낚이면 시간 손실 또는 스트라이크가 누적됩니다.</p>
        </VintagePanel>
        <VintagePanel title="짧은 한 판" eyebrow="Rule 03">
          <p>한 세션은 약 2~3분, 데스크톱 브라우저에서 바로 다시 도전할 수 있습니다.</p>
        </VintagePanel>
      </section>

      <section className="landing-bottom-grid">
        <VintagePanel title="대표 미션 예시" eyebrow="Mission Preview">
          <ul className="bullet-list">
            <li>메일 첨부 전에 진짜 보안 경고만 처리하기</li>
            <li>가짜 업데이트 창을 무시하고 머지 충돌 해결하기</li>
          </ul>
        </VintagePanel>
        <VintagePanel title="실패 조건" eyebrow="Watch Out">
          <ul className="bullet-list">
